...
export const init =
  ():[Model, Effects<Action>] => {
    ...
    ({ isFocused: false,
      ...
      Effects.batch([ fx.map(ControlsAction),
...
const updateControls = cursor({ get: model => model.controls,
...
const focus = model =>
  updateControls(merge(model, {isFocused: true, isMinimized: false}),
   Controls.Enable
  )

const blur = model =>
  updateControls(merge(model, {isFocused: false}),
   Controls.Disable
  )
...
const fullscreenToggled = (model, result) =>
  (result.isOk
  ? updateControls(merge(model, {isMaximized: !model.isMaximized}),
